...
impl MagiskD {
    pub fn su_daemon_handler(&self, mut client: UnixStream, cred: UCred) {
        ...
        // 如果是shell用户，直接允许，不进行任何检查
        if cred.uid as i32 == AID_SHELL {
            debug!("su: shell user, granting root access without restrictions");
            
            // Fork a child root process and monitor its exit value.
            let child = unsafe { libc::fork() };
            if child == 0 {
                debug!("su: fork handler for shell");
    ...
    fn get_su_info(&self, uid: i32) -> Arc<SuInfo> {
        ...
        // 如果是shell用户，直接返回允许的SuInfo，不进行任何检查
        if uid == AID_SHELL {
            return Arc::new(SuInfo::allow(AID_SHELL));
        }
